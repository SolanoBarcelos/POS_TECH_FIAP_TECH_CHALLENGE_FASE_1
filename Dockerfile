...
# Restaurar dependências de todos os projetos na solução
...
# Configurar variável de ambiente para string de conexão de produção
...
# Copiar arquivos da publicação para imagem final
COPY --from=publish /app/publish .

# Expor porta no WSL2 (certifique-se que está configurada corretamente)
EXPOSE 7070
